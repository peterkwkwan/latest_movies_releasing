...
function App() {
  ...
  const fetchSearchMovie =  () => {
   fetch(fetchSearchMovieUrl).then(res => res.json()).then(res => setSearchResults(res.results))
  }
  ...
  return (
    ...
    { page !== 1 && <button type="button" onClick={handlePreviousClicked}>Previous Page</button>}
    <button type="button" onClick={handleNextClicked}>Next Page</button>
